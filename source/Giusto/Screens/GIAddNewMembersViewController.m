//
//  GIAddNewMembersViewController.m
//  Giusto
//
...
@interface GIAddNewMembersViewController () <UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate>
...
@implementation GIAddNewMembersViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BOOL inCurrentTable = [[self.tableProfiles filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"SELF.parseObject.objectId == %@", selectedProfile.parseObject.objectId]] count];
    ...
    if (inCurrentTable) {
        ...
        else
        {
            ...
            [selectedProfile dependentsWithCompletion:^(id sender, BOOL success, NSError *error, NSArray *dependents) {
                if (dependents.count) {
                    NSArray *mergedProfiles = [self.addedProfiles arrayByAddingObjectsFromArray:self.tableProfiles];
                    
                    for (GIUserProfile *dependent in dependents) {
                        if ([[mergedProfiles filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"SELF.parseObject.objectId == %@", dependent.parseObject.objectId]] count] && ![[self.removedProfiles filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"SELF.parseObject.objectId == %@", dependent.parseObject.objectId]] count]) {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    destination.selectedUserProfile = selectedUser.userProfile;
    destination.userProfiles = self.tableProfiles;
...
- (void)configureCell:(GIUserProfileTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    BOOL inCurrentTable = [[self.tableProfiles filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"SELF.parseObject.objectId == %@", userProfile.parseObject.objectId]] count];
